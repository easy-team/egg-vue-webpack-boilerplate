  ...
  <div style="height:100%">
    <header class="admin-header">
      <div class="admin-logo">
        <span :class="{'admin-big': !collapse, 'admin-min': collapse }">{{ collapse ? site.name : site.description }}</span>
      </div>
      <span 
        class="admin-header-btn" 
        @click="sidebarToggle"><i class="el-icon-menu"/></span>
       
      <div class="right">
        <span class="admin-header-btn">
          <a :href="$t('lang.href')"><i class="el-icon-message"/></a>
        </span>
        <el-dropdown>
          <span class="admin-header-btn">
            {{ $t('lang.text') }}<i class="el-icon-arrow-down el-icon--right"/>
          </span>
